...
<head>
    ...
    <style>
        ...
        .container { max-width: 800px; margin: auto; background: white; border-radius: 12px; padding: 30px; box-shadow: 0 8px 20px rgba(0,0,0,0.1); }
        h1 { color: #2c3e50; border-bottom: 2px solid #3498db; padding-bottom: 10px; }
        h2 { color: #3498db; margin-top: 30px; }
        pre { background: #f8f9fa; padding: 15px; border-radius: 8px; overflow-x: auto; }
        code { background: #e9ecef; padding: 2px 6px; border-radius: 4px; }
        .endpoint { background: #e9ecef; padding: 10px; border-radius: 6px; margin: 20px 0; }
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not parser_type or not input_data:
            self.send_response(400)
            self.end_headers()
            self.wfile.write(json.dumps({"error": "Missing parser or input"}).encode())
            return

        # If input is a URL and parser is not 'direct', fetch it
